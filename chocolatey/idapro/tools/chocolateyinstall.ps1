...
$PASSWORD_FILE = "ida-password.txt"
$INSTALLER_FILE = Get-Item -Path "*" -Filter "idaprocw*.exe"


if(Test-Path $PASSWORD_FILE) {
  $password = Get-Content -Path $PASSWORD_FILE
}
...
$packageArgs = @{
  ...
  file64          = $INSTALLER_FILE.FullName

  softwareName  = 'idapro*' #part or all of the Display Name as you see it in Programs and Features. It should be enough to be unique

  # Checksums are now required as of 0.10.0.
  # To determine checksums, you can get that from the original site if provided. 
  # You can also use checksum.exe (choco install checksum) and use it 
  # e.g. checksum -t sha256 -f path\to\file
  #checksum      = 'BFF89DBC7D68D0BC6B8FA04AE8A2C788C2CA0D5DB5B53B3B6B2826532CCED9F4'
  #checksumType  = 'sha256' #default is md5, can also be sha1, sha256 or sha512

  # MSI
  silentArgs    = "/LOG /SP- /SILENT /SUPPRESSMSGBOXES /NORESTART /PASSWORD=$($password)"
  validExitCodes= @(0)
  # OTHERS
